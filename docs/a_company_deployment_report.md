...
A社社内に Inquira を **HTTPS / Google OAuth 認証付き** で導入完了しました。
管理者は社内 LAN から `https://inquira.example.local/` でアクセスし、登録済み Gmail でログイン可能です。

- ✅ サーバー上に Inquira (Python/FastAPI) 起動
- ✅ 社内 DNS に FQDN (`inquira.example.local`) 登録
- ✅ IIS リバースプロキシで HTTPS 化 (自己署名証明書)
- ✅ Google OAuth 連携 (管理者 4 名のホワイトリスト方式)
- ✅ データ保存先を社内ファイルサーバー (`\\fileserver\share\…`) に統一 (バックアップ既存運用に乗る)
...
[社員 PC (Edge/Chrome)]
        ...
        ▼
[社内 DNS: AD-SERVER-EXAMPLE]
        │  A レコード解決
        │  inquira.example.local → 10.0.0.10
        ▼
[Inquira サーバー: 10.0.0.10]
   ├─ IIS (443/TCP)
   │    ├─ 自己署名 SSL 証明書
   │    │  Thumbprint: REDACTED_CERT_THUMBPRINT
   │    │  有効期限: 2031-06-14
   │    └─ URL Rewrite + ARR (リバースプロキシ)
   │         ↓ http://127.0.0.1:8000/ にフォワード
   │         X-Forwarded-Proto: https
   │         X-Forwarded-Host: inquira.example.local
   │
   ├─ Inquira (uvicorn / Python 3.11) : 127.0.0.1:8000
   │    └─ Google OAuth 認証 → 管理画面・Q&A 機能
   │
   └─ \\fileserver\share\システム\inquira_share\
...
| 登録済み管理者 | 4 名 (Google Cloud Console テストユーザー登録済み) |
...
A社サーバーは **物理オンプレ / Docker 不可 / 管理者権限制約あり / 社内 DNS は別管理者** という条件で、いくつかのハマりがありました。最終的に下記の構成で安定運用に到達しました。
...
| `http://192.168.x.x:8000/` で運用 | Google OAuth が **IP リダイレクト URI を拒否** | 社内 DNS で FQDN 払い出し (`inquira.example.local`) |
| HTTP (port 8000) で運用 | OAuth 状態 Cookie が `SameSite=Lax` で破損、Edge から不安定 | **IIS + HTTPS リバプロ** で `https://inquira.example.local/` に統一 |
| データはサーバーローカル保存 | A社既存バックアップが共有領域のみ | 共有 `\\fileserver\share\システム\inquira_share\` 配下に統一 |
...
| 5 | `start_inquira.bat` 実行しても古い設定で起動 | 旧 Python プロセスが残ったまま新プロセス起動 → 旧の方が応答 | 再起動手順を `Get-Process python \| Stop-Process -Force` から開始するよう統一 |
| 6 | `https://inquira.example.local/` がタイムアウトでつながらない | **DNS A レコードが AD サーバー (10.0.0.20)** を指していて、Inquira サーバー (10.0.0.10) と別マシンだった | `lessons_learned` に登録 + AD_REQUEST.md / setup スクリプトに **「Inquira サーバーの実 IP を確認」チェックを追加** (TODO) |
| 7 | Google ログインボタンを押すと `{"detail":"Not Found"}` | **IIS ARR の `reverseRewriteHostInResponseHeaders`** (既定 true) が Location ヘッダの `accounts.google.com` を `inquira.example.local` に書き換えていた | `setup_iis_reverse_proxy.ps1` v2 で **`false` に設定する処理を自動化** (今後の顧客では再発しない) |
...
| # | 課題 | 推奨対応 | 担当 |
|---|---|---|---|
| 1 | 自己署名証明書のブラウザ警告 | 社員 PC の Edge/Chrome では「保護されていない通信」警告が出る。AD のグループポリシーで証明書をルートストアに配布すれば解消 | A社 IT 部門 |
| 2 | OAuth 同意画面がテストモード | 100 人を超えるユーザーで使う場合は **本番モードに昇格** が必要 (Google 審査あり) | 提供側 |
| 3 | 管理者は現在 4 名のみ | 追加したいときは Google Cloud Console の「テストユーザー」と `.env` の `ALLOWED_EMAILS` の両方に追加 | 提供側 |
...
| 5 | バックアップ確認 | `\\fileserver\share\システム\inquira_share\` が既存のファイルサーバーバックアップ対象に含まれているか確認 |
| 6 | 監視 / アラート | `https://inquira.example.local/healthz` を社内監視に登録 (Zabbix/PRTG 等) |
| 7 | 全社員開放 | `.env` に `ALLOWED_DOMAIN=example.local` を追加すれば、ホワイトリスト無しで全社員ログイン可能 |
...
例: 管理者を追加するには:
```
ALLOWED_EMAILS=admin1@gmail.com,admin2@gmail.com,新規追加@gmail.com
```
...
| 質問履歴 (監査) | `\\fileserver\share\システム\inquira_share\audit\audit-YYYY-MM-DD.jsonl` |
...
| 文書 | 用途 |
|---|---|
| `tenants/a_company/README.md` | A社向け導入手順書 (IT 部門・管理者向け) |
| `docs/a_company_admin_quickstart.pdf` | 管理者向けクイックスタート (ナレッジ投入) |
| `docs/a_company_user_quickstart.pdf` | 一般社員向け利用ガイド |
...
| `docs/deployment_lessons_learned.md` | 今回の経験を次の顧客導入に活かすための社内ナレッジ |
...
| 項目 | 状態 |
|---|---|
| API キー (Anthropic) | A社用に新規発行、`.env` のみに保管 (Git 管理外) |
| Google OAuth Client Secret | 同上 |
| `.env` ファイル | `\\fileserver\share\…` ではなく **サーバーローカル (`%USERPROFILE%\Inquira\.env`)** に保管 |
...
導入後の運用、ナレッジ拡張、新機能追加等のご相談は提供側まで。
